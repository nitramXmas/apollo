...
const Planet = ({ planet, moons, indexObject, indexAstre }) => {
  let { meanRadius, aphelion, sideralRotation, axialTilt, sideralOrbit } =
    planet; //let because we modify some value for scale
      ...
      <mesh ref={turnArroundSun}>
        ...
        {moons &&
          moons.map((moon) => (
            <Center
              key={moon.id}
              position={[aphelion + moon.aphelion / 200000, 0, 0]}
            >
              <Moon moon={moon} />
            </Center>
          ))}
