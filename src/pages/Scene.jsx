...
const Scene = () => {
  ...
  return (
    <div className="scene">
      ...
      <div className="canvas">
        {isLoading ? (
          <div className="loading">
            <img
              className="capybara"
              src="https://i.redd.it/capy-is-king-v0-ian6ahoqhqna1.png?s=5639c3866298be9ecffdfe3c8f34f9f8371e885e"
              alt="capybara"
            />
            <span>Loading</span>
          ...
          <>
              ...
              <div className="card-list">
                {objects &&
                  objects
                    .filter(
                      (object, index) =>
                        (object.bodyType === "Planet" &&
                          index === indexObject) ||
                        (object.bodyType === "Star" && index === indexObject) ||
                        (object.bodyType === "Moon" && index === indexObject)
                    )
                    .map((planet) => {
                      return (
                        <Card
            ...
            <Canvas shadows>
              ...
              <pointLight intensity={0.5} castShadow />

              {objects &&
                objects.map((astre, indexAstre) => {
                  if (astre.bodyType === "Star")
                    return <Sun key={astre.id} sun={astre} />;
                  if (astre.bodyType === "Planet")
                    return (
                      <Planet
                        key={astre.id}
                        planet={astre}
      ...
      <Navbar handleSetObject={handleSetObject} handleClicked={handleClicked} />
    </div>
  );
};
